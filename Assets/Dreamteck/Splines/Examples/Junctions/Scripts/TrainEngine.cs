namespace Dreamteck.Splines.Examples
{
    ...
    public class TrainEngine : MonoBehaviour
    {
        ...
        public string getState(string name){
            ...
            restClient rClient = gameObject.AddComponent<restClient>();
            rClient.endPoint = uri;

            string strResponse = string.Empty;

            strResponse = rClient.makeRequest();
            JsonTextReader reader = new JsonTextReader(new StringReader(strResponse));

            string state = null;
            while (reader.Read())
            {
                if (reader.Value != null && reader.Value.ToString() != "state" && reader.Value.ToString() != "id")
                {   
        ...
        private void OnJunction(List<SplineTracer.NodeConnection> passed)
        {
            ...
            string state = getState(node.name);
            ...
            {
                if(state == "CLOSED") bridge.active = false;
                if(state == "THROWN") bridge.active = true;
